...
func main() {
	...
	// Wrap base repos with a Cached Proxy (acts as cache+persist proxy). In this sample, base repos are in-memory;
	// in a real app, you would wrap DB repos. Facades accept repository.ICommonRepo, so we can pass cached or base.
	...
	for {
		...
		case "4":
			...
			_, err := catF.CreateCategory(name, category.CategoryType(t))
			if err != nil {
				fmt.Println("error:", err)
			} else {
				fmt.Println("created")
		...
		case "6":
			...
			if _, err := opF.CreateOperation(operation.OperationType(t), service.ObjectID(accID), amount, date, service.ObjectID(catID), descr); err != nil {
				fmt.Println("error:", err)
			} else {
				fmt.Println("created")
			}
